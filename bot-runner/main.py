...
django.conf.settings.configure(
    DATABASES=settings.DATABASES, INSTALLED_APPS=settings.INSTALLED_APPS,
)
django.setup()
...
MSG_WELCOME_WAIT_FOR_APROBE = 'Welcome {{ first_name }}, you must be approved by an administrator'  # noqa
...
def handlerProcessor(handler, **kwargs):
    ...
    if handler.payload['type'] == 'message':
        ...
        raise ValueError(
            f"Unsupported endpoint type: {handler.payload['type']}",
        )
    ...
    if command == '/start':
        ...
        if reg:
            if bot.response_all:
                ...
                t = TextProccesor(
                    MSG_WELCOME_WAIT_FOR_APROBE, message, context,
                )
                t.execute()
                return
        else:
                ...
                if handler.enabled:
                    if bot.response_all:
                        ...
                        if user.response_enabled:
                            ...
                            t = TextProccesor(
                                MSG_USER_DISABLED, message, context,
                            )
                            t.execute()
                            return
                else:
